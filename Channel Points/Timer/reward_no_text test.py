...
video_out = cv2.VideoWriter("word_ban.avi", fourcc, fps, (width, height))
...
for i in range(int(fps * countdown_seconds + 10)):
    ...
    text_size = cv2.getTextSize("IRL word Ban".upper(), font, font_size_text, font_weight)[0]
    text_position = ((width - text_size[0]) // 2, int(height * height_text))
    cv2.putText(frame, "IRL word ban".upper(), text_position, font, font_size_text, color, font_weight)
